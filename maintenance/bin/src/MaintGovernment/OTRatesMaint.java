...
public class OTRatesMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;

	static String DEFAULT_QUERY = DBConnect.Select("otrates ",
			 						"otrates_code, "+					
			 						"otrates_desc, "+					
...
//	static String DEFAULT_QUERY = "SELECT otrates_code,otrates_desc," +
//								  "FORMAT(otrates_rate,2) AS rates,comps_code "+
//								  "FROM otrates "+ 
//								  "LEFT JOIN compensation on otrates.comps_code = compensation.comps_code";
	...
	public OTRatesMaint() {
		...
		try {
			...
			tableOTRates.resultSet = tableOTRates.statement.executeQuery(sql);

			jcbCompensation.setFont(new Font("",Font.PLAIN, 12));
			jcbCompensation.setBackground(Color.white);
			jcbCompensation.addItem("Choose");
			while (tableOTRates.resultSet.next()){
				jcbCompensation.addItem(Formatter.formatString(150,' ','-',tableOTRates.resultSet.getString("comps_desc"))+"-"+tableOTRates.resultSet.getString("comps_code"));
			...
			//set display size
			scrollPane.setPreferredSize(new Dimension(557,170));
			jpan.setBounds(10,20,557,180);

			lblcode.setBounds(28,100,100,280);
			lbldescription.setBounds(28,130,100,280);
			lblrate.setBounds(28,160,100,280);
			lblcompensation.setBounds(28,190,100,280);
			...
			jcbCompensation.setBounds(120,320,380,20);
			
			jbtnadd.setBounds(100,200,80,22);
			jbtnedit.setBounds(250,200,80,22);
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e) {
					...
					if (jcbCompensation.getSelectedIndex() > 0) {
						comp = jcbCompensation.getSelectedItem().toString().substring(151).trim();
					}
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("otrates",
								"otrates_code,otrates_desc,otrates_rate,comps_code",
						...
						try {
							if (xcode.length() == 0) {   
								...
								tbl.getColumnModel().getColumn(3).setHeaderValue("Compensation");
							...
							public void run() {
								try {
									...
									LockHandler.startLock(DBConnect.SelectForUpdate("otrates",
											"*",
											"otrates_code = '"+tbl.getValueAt(tbl.getSelectedRow(),0).toString()+"'"));

									String del_sql = DBConnect.delete("otrates","otrates_code = '"+DBConnect.clean(xcode)+"'");
		...
		if (tbl.getSelectedRow() >= 0) {
			...
			for (int ii=0; ii<=jcbCompensation.getItemCount()-1; ii++) {
				if (jcbCompensation.getItemAt(ii).toString().length() > 50) {
					if (tbl.getValueAt(getrow,3).toString().equalsIgnoreCase(jcbCompensation.getItemAt(ii).toString().substring(151)) == true) {
						jcbCompensation.setSelectedIndex(ii);
					}
				}
			}//ENDof for (int ii=0; ii<=jcbCompensation.getItemCount()-1; ii++) {
