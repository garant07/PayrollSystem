 ...
 * Program: WTax Status Maintenance ==-> Payroll System
 */
...
public class WTaxStatusMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("wtaxstatus",
														 "wtxstat_code, wtxstat_desc",
														 null,
														 "wtxstat_code ASC");
	...
	private ResultSetTableModel tableWTaxStatus;
	Container container;
	JTable tbl;
	...
	public WTaxStatusMaint(){
		super ("WTax Status Maintenance", false, true, false, false);
		this.setName("wtaxstatus");
		...
		try{
			...
			tableWTaxStatus  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tbl = new JTable(tableWTaxStatus);
			...
			tableWTaxStatus.resultSet = tableWTaxStatus.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("wtaxstatus",
								"wtxstat_code,wtxstat_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into wtaxstatus (wtaxstatus.code,wtaxstatus.desc,wtaxstatus.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tableWTaxStatus.setInsert(save_sql);

								tableWTaxStatus.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("wtaxstatus","*","wtxstat_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("wtaxstatus",
											"wtxstat_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from wtaxstatus where code = '"+xcode+"'";
									...
									if (txtcode.getText().length() != 0) {
										...
										if(setDialog == JOptionPane.YES_OPTION){
											try {
												...
												//tableWTaxStatus.setInsert(del_sql);

												tableWTaxStatus.setQuery(DEFAULT_QUERY);
												int iColumn = 0;

												//if (xcode != "Code - WTaxStatus"){
												ListSelectionModel selectionModel = tbl.getSelectionModel();
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("wtaxstatus",
										"wtxstat_desc='"+DBConnect.clean(udesc)+"'",
										"wtxstat_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("WTaxStatusMaint", "WTaxStatusMaint", traceWriter.toString());      
			dispose();
		}
