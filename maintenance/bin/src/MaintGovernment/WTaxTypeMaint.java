 ...
 * Program: WithHolding Tax Type Maintenance ==-> Payroll System
 */
...
public class WTaxTypeMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("wtaxtype",
														 "wtxtype_code, wtxtype_desc",
														 null,
														 "wtxtype_code ASC");
	public static int subMain = 0;

	public static JInternalFrame iFrame = null;
	private ResultSetTableModel tableWTaxType;
	Container container;
	JTable tbl;
	...
	public WTaxTypeMaint(){
		super ("WithHolding Tax Type Maintenance", false, true, false, false);
		this.setName("wtaxtype");
		...
		this.addInternalFrameListener(new InternalFrameListener(){
			public void internalFrameActivated(InternalFrameEvent arg0) {
				if (subMain == 1) {
					try {
						WTaxTableMaint.iFrame.setSelected(true);
					}
		...
		try{
			...
			tableWTaxType  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tableWTaxType.resultSet = tableWTaxType.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("wtaxtype",
								"wtxtype_code,wtxtype_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into wtaxtype (wtaxtype.code,wtaxtype.desc,wtaxtype.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tableWTaxType.setInsert(save_sql);

								tableWTaxType.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("wtaxtype","*","wtxtype_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("wtaxtype",
											"wtxtype_code = '"+DBConnect.clean(xcode)+"'");
									String del_sql1 = "DELETE FROM wtax WHERE wtax_code = '"+xcode+"'";
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("wtaxtype",
										"wtxtype_desc='"+DBConnect.clean(udesc)+"'",
										"wtxtype_code= '"+DBConnect.clean(ucode)+"'");
				...
				public void actionPerformed(ActionEvent e) {
					if (tbl.getRowCount() >= 1) {
						if (tbl.getSelectedRow() >= 0) {
							int getrow = tbl.getSelectedRow();
							new WTaxTableMaint(tbl.getValueAt(getrow,0).toString(),tbl.getValueAt(getrow,1).toString(),Width(),Height());
						}
					}
				}
			});
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("WTaxTypeMaint", "WTaxTypeMaint", traceWriter.toString());      
			dispose();
		}
