 ...
 * Program: PhilHealth Type Maintenance ==-> Payroll System
 */
...
public class PhilHealthTypeMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("philhealthtype",
														 "phhtype_code, phhtype_desc",
														 null,
														 "phhtype_code ASC");
	public static int subMain = 0;
	
	private ResultSetTableModel tablePhilHealthType;
	Container container;
	JTable tbl;
	...
	public PhilHealthTypeMaint(){
		super ("PhilHealth Type Maintenance", false, true, false, false);
		this.setName("phhtype");
		...
		this.addInternalFrameListener(new InternalFrameListener(){
			public void internalFrameActivated(InternalFrameEvent arg0) {
				if (subMain == 1) {
					try {
						PhilHealthMaint.iFrame.setSelected(true);
					}
		...
		try{
			...
			tablePhilHealthType  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			jbtntable = new JButton("Table");
			jbtntable.setMnemonic('T');
			
			JLabel lblcode = new JLabel("Code");
			final JLabel lbldescription = new JLabel("Description");
			tbl = new JTable(tablePhilHealthType);
			...
			tablePhilHealthType.resultSet = tablePhilHealthType.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtntable.setBounds(330,170,80,22);
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						jbtntable.setEnabled(false);
						txtcode.grabFocus();
					}
					else if (caption == "Save"){
						String save_sql = DBConnect.Insert("philhealthtype",
								"phhtype_code,phhtype_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into phhtype (phhtype.code,phhtype.desc,phhtype.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tablePhilHealthType.setInsert(save_sql);

								tablePhilHealthType.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("philhealthtype","*","phhtype_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("philhealthtype",
											"phhtype_code = '"+DBConnect.clean(xcode)+"'");
									String del_sql1 = "DELETE FROM philhealth WHERE phhtype_code = '"+xcode+"'";
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("philhealthtype",
										"phhtype_desc='"+DBConnect.clean(udesc)+"'",
										"phhtype_code= '"+DBConnect.clean(ucode)+"'");
				...
				public void actionPerformed(ActionEvent e) {
					if (tbl.getRowCount() >= 1) {
						if (tbl.getSelectedRow() >= 0) {
							int getrow = tbl.getSelectedRow();
							new PhilHealthMaint(tbl.getValueAt(getrow,0).toString(),tbl.getValueAt(getrow,1).toString(),Width(),Height());
						}
					}
				}
			});
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("PhilHealthTypeMaint", "PhilHealthTypeMaint", traceWriter.toString());      
			dispose();
		}
