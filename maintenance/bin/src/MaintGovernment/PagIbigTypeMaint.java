 ...
 * Program: PagIbig Type Maintenance ==-> Payroll System
 */
...
public class PagIbigTypeMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("pagibigtype",
														 "pbgtype_code, pbgtype_desc",
														 null,
														 "pbgtype_code ASC");
	public static int subMain = 0;
	
	private ResultSetTableModel tablePagIbigType;
	Container container;
	JTable tbl;
	...
	public PagIbigTypeMaint(){
		super ("HDMF Type Maintenance", false, true, false, false);
		this.setName("pbgtype");
		...
		this.addInternalFrameListener(new InternalFrameListener(){
			public void internalFrameActivated(InternalFrameEvent arg0) {
				if (subMain == 1) {
					try {
						PagIbigMaint.iFrame.setSelected(true);
					}
		...
		try{
			...
			tablePagIbigType  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tbl = new JTable(tablePagIbigType);
			...
			tablePagIbigType.resultSet = tablePagIbigType.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("pagibigtype",
								"pbgtype_code,pbgtype_desc",
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tablePagIbigType.setInsert(save_sql);

								tablePagIbigType.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("pagibigtype","*","pbgtype_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("pagibigtype",
											"pbgtype_code = '"+DBConnect.clean(xcode)+"'");
									String del_sql1 = "DELETE FROM pagibig WHERE pbgtype_code = '"+xcode+"'";
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("pagibigtype",
										"pbgtype_desc='"+DBConnect.clean(udesc)+"'",
										"pbgtype_code= '"+DBConnect.clean(ucode)+"'");
				...
				public void actionPerformed(ActionEvent e) {
					if (tbl.getRowCount() >= 1) {
						if (tbl.getSelectedRow() >= 0) {
							int getrow = tbl.getSelectedRow();
							new PagIbigMaint(tbl.getValueAt(getrow,0).toString(),tbl.getValueAt(getrow,1).toString(),Width(),Height());
						}
					}
				}
			});
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("PagIbigTypeMaint", "PagIbigTypeMaint", traceWriter.toString());      
			dispose();
		}
