 ...
 * Program: SSS Type Maintenance ==-> Payroll System
 */
...
public class SSSTypeMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("ssstype",
														 "ssstype_code, ssstype_desc",
														 null,
														 "ssstype_code ASC");
	public static int subMain = 0;

	private ResultSetTableModel tableSSSType;
	Container container;
	JTable tbl;
	...
	public SSSTypeMaint(){
		super ("SSS Type Maintenance", false, true, false, false);
		this.setName("ssstype");
		...
		this.addInternalFrameListener(new InternalFrameListener(){
			public void internalFrameActivated(InternalFrameEvent arg0) {
				if (subMain == 1) {
					try {
						SSSMaint.iFrame.setSelected(true);
					}
		...
		try{
			...
			tableSSSType  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tableSSSType.resultSet = tableSSSType.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("ssstype",
								"ssstype_code,ssstype_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into ssstype (ssstype.code,ssstype.desc,ssstype.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tableSSSType.setInsert(save_sql);

								tableSSSType.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("ssstype","*","ssstype_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("ssstype",
											"ssstype_code = '"+DBConnect.clean(xcode)+"'");
									String del_sql1 = "DELETE FROM sss WHERE ssstype_code = '"+xcode+"'";
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("ssstype",
										"ssstype_desc='"+DBConnect.clean(udesc)+"'",
										"ssstype_code= '"+DBConnect.clean(ucode)+"'");
				...
				public void actionPerformed(ActionEvent e) {
					if (tbl.getRowCount() >= 1) {
						if (tbl.getSelectedRow() >= 0) {
							int getrow = tbl.getSelectedRow();
							new SSSMaint(tbl.getValueAt(getrow,0).toString(),tbl.getValueAt(getrow,1).toString(),Width(),Height());
						}
					}
				}
			});
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("SSSTypeMaint", "SSSTypeMaint", traceWriter.toString());      
			dispose();
		}
