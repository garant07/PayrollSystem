package maintenance.bin.src.Country;

/**
 * Author : Norberto L. Silva
 * Date   : June 29, 2009
 * Company: Applied Ideas, Inc 
 * Program: Country Maintenance
 */
...
public class Country extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("position",
			"position.code AS Code, position.desc AS Descripiton, position.authority AS Level",
			null,
			"code ASC");
	...
	private MyField txtcode;
	private MyField txtdescription;
	private MyField txtauth;
	 
	public Country() {
		super ("Country Maintenance", false, true, false, false);
            ...
            new ActionListener()
            {
				...
				public void actionPerformed(ActionEvent e)
            	{
                        ...
                        if (caption == "Add"){
                           ...
                           String save_sql = DBConnect.Insert("position",
                        		   "position.code,position.desc,position.authority",
                           ...
                           txtcode.grabFocus();
                        	   ...
                        	   Message.messageError("Record Already Exists !!");
                        	   }catch(Exception exc){
        ...
        );//jbtnadd.addMouseListener(
        
        //delete button listener
        jbtndelete.addActionListener(
            new ActionListener()
            {
            	//@SuppressWarnings("deprecation")
				public void actionPerformed(ActionEvent e)
    							...
    							public void run() {
    								try {
    									...
    									String lockCmd = DBConnect.SelectForUpdate
    										("position","*","code = '" + DBConnect.clean(code) +"'");
    									LockHandler.startLock(lockCmd);
    									
    									String del_sql = DBConnect.delete("position",
        ...
        );//jbtndelete.addMouseListener(
        
        //edit button listener
        jbtnedit.addActionListener(
            new ActionListener()
            {
            	//@SuppressWarnings("deprecation")
				public void actionPerformed(ActionEvent e)
...
//                        	 lock indicator 
                        	...
                        	try{
                        			String e_sql = DBConnect.Update("position",
                        					"position.desc='"+DBConnect.clean(udesc)+"', authority='"+DBConnect.clean(uauth)+"'",
                        					
                        					"code= '"+DBConnect.clean(ucode)+"'");
                        		   //String e_sql = "Update position SET position.desc='"+udesc+"', authority='"+uauth+"' Where code= '"+ucode+"'";
...
//				System.out.println("hello2 "+eventKey);
	      ...
	      catch ( ClassNotFoundException classNotFound ) 
	      {
			  ...
			  createINI.create("PositionMaint", "PositionMaint", traceWriter.toString());      
		      dispose();
	      }
...
}//ENDof public class Country extends JInternalFrame {
