 ...
 * Program: Payment Type Maintenance ==-> Payroll System
 */
...
public class PaymentTypeMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("paymenttype",
														 "pymnt_code, pymnt_desc",
														 null,
														 "pymnt_code ASC");

	private ResultSetTableModel tablePaymentType;
	Container container;
	JTable tbl;
	...
	private MyField txtdescription = new MyField(true,10);

	public PaymentTypeMaint(){
		super ("Payment Type Maintenance", false, true, false, false);
		...
		try{
			...
			tablePaymentType  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tbl = new JTable(tablePaymentType);
			...
			tablePaymentType.resultSet = tablePaymentType.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("paymenttype",
								"pymnt_code,pymnt_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into paymenttype (paymenttype.code,paymenttype.desc,paymenttype.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tablePaymentType.setInsert(save_sql);

								tablePaymentType.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("paymenttype","*","pymnt_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("paymenttype",
											"pymnt_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from paymenttype where code = '"+xcode+"'";
									...
									if (txtcode.getText().length() != 0) {
										...
										if(setDialog == JOptionPane.YES_OPTION){
											try {
												...
												//tablePaymentType.setInsert(del_sql);

												tablePaymentType.setQuery(DEFAULT_QUERY);
												int iColumn = 0;

												//if (xcode != "Code - PaymentType"){
												ListSelectionModel selectionModel = tbl.getSelectionModel();
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("paymenttype",
										"pymnt_desc='"+DBConnect.clean(udesc)+"'",
										"pymnt_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("PaymentTypeMaint", "PaymentTypeMaint", traceWriter.toString());      
			dispose();
		}
