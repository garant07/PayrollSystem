 ...
 * Program: Loan Type Maintenance ==-> Payroll System
 */
...
public class LoanTypeMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("loantype",
														 "lntype_code, lntype_desc",
														 null,
														 "lntype_code ASC");

	private ResultSetTableModel tableLoanType;
	Container container;
	JTable tbl;
	...
	public LoanTypeMaint(){
		...
		try{
			...
			tableLoanType  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tableLoanType.resultSet = tableLoanType.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("loantype",
								"lntype_code,lntype_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into loantype (loantype.code,loantype.desc,loantype.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tableLoanType.setInsert(save_sql);

								tableLoanType.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("loantype","*","lntype_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("loantype",
											"lntype_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from loantype where code = '"+xcode+"'";
									...
									if (txtcode.getText().length() != 0) {
										...
										if(setDialog == JOptionPane.YES_OPTION){
											try {
												...
												//tableLoanType.setInsert(del_sql);

												tableLoanType.setQuery(DEFAULT_QUERY);
												int iColumn = 0;

												//if (xcode != "Code - LoanType"){
												ListSelectionModel selectionModel = tbl.getSelectionModel();
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("loantype",
										"lntype_desc='"+DBConnect.clean(udesc)+"'",
										"lntype_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("LoanTypeMaint", "LoanTypeMaint", traceWriter.toString());      
			dispose();
		}
