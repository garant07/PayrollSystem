 ...
 * Program: Department Maintenance ==-> Payroll System
 */
...
public class DepartmentMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("department",
														 "dept_code, dept_desc",
														 null,
														 "dept_code ASC");
	...
	private ResultSetTableModel tabledept;
	Container container;
	JTable tbl;
	...
	public DepartmentMaint(){
		super ("Department Maintenance", false, true, false, false);
		...
		try{
			...
			tabledept  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tabledept.resultSet = tabledept.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("department",
								"dept_code,dept_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into department (department.code,department.desc,department.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tabledept.setInsert(save_sql);

								tabledept.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("department","*","dept_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("department",
											"dept_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from department where code = '"+xcode+"'";
									...
									if (txtcode.getText().length() != 0) {
										...
										if(setDialog == JOptionPane.YES_OPTION){
											try {
												...
												//tabledept.setInsert(del_sql);

												tabledept.setQuery(DEFAULT_QUERY);
												int iColumn = 0;

												//if (xcode != "Code - Department"){
												ListSelectionModel selectionModel = tbl.getSelectionModel();
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("department",
										"dept_desc='"+DBConnect.clean(udesc)+"'",
										"dept_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("DepartmentMaint", "DepartmentMaint", traceWriter.toString());      
			dispose();
		}
