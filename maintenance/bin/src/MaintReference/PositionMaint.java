 ...
 * Program: Position Maintenance ==-> Payroll System
 */
...
public class PositionMaint extends JInternalFrame {
	...
	public PositionMaint(){
		super ("Position Maintenance", false, true, false, false);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("position","*","post_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("position",
											"post_code = '"+DBConnect.clean(xcode)+"'");
						...
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("position",
										"post_desc='"+DBConnect.clean(udesc)+"'",
										"post_code= '"+DBConnect.clean(ucode)+"'");
