 ...
 * Program: Payroll Type Maintenance ==-> Payroll System
 */
...
public class PayrollTypeMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("payrolltype",
														 "ptype_code, ptype_desc",
														 null,
														 "ptype_code ASC");

	private ResultSetTableModel tablePayrollType;
	Container container;
	JTable tbl;
	...
	public PayrollTypeMaint(){
		super ("Payroll Type Maintenance", false, true, false, false);
		this.setName("payrolltype");
		...
		try{
			...
			tablePayrollType  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tbl = new JTable(tablePayrollType);
			...
			tablePayrollType.resultSet = tablePayrollType.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("payrolltype",
								"ptype_code,ptype_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into payrolltype (payrolltype.code,payrolltype.desc,payrolltype.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tablePayrollType.setInsert(save_sql);

								tablePayrollType.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("payrolltype","*","ptype_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("payrolltype",
											"ptype_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from payrolltype where code = '"+xcode+"'";
									...
									if (txtcode.getText().length() != 0) {
										...
										if(setDialog == JOptionPane.YES_OPTION){
											try {
												...
												//tablePayrollType.setInsert(del_sql);

												tablePayrollType.setQuery(DEFAULT_QUERY);
												int iColumn = 0;

												//if (xcode != "Code - PayrollType"){
												ListSelectionModel selectionModel = tbl.getSelectionModel();
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("payrolltype",
										"ptype_desc='"+DBConnect.clean(udesc)+"'",
										"ptype_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("PayrollTypeMaint", "PayrollTypeMaint", traceWriter.toString());      
			dispose();
		}
