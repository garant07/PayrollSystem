 ...
 * Program: Payroll Process Type Maintenance ==-> Payroll System
 */
...
public class PayrollProcessTypeMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("payrollprocesstype",
														 "pptype_code, pptype_desc",
														 null,
														 "pptype_code ASC");

	private ResultSetTableModel tablePayrollProcessType;
	Container container;
	JTable tbl;
	...
	public PayrollProcessTypeMaint(){
		super ("Payroll Process Type Maintenance", false, true, false, false);
		this.setName("payrollprocesstype");
		...
		try{
			...
			tablePayrollProcessType  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tbl = new JTable(tablePayrollProcessType);
			...
			tablePayrollProcessType.resultSet = tablePayrollProcessType.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("payrollprocesstype",
								"pptype_code,pptype_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into payrollprocesstype (payrollprocesstype.code,payrollprocesstype.desc,payrollprocesstype.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tablePayrollProcessType.setInsert(save_sql);

								tablePayrollProcessType.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("payrollprocesstype","*","pptype_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("payrollprocesstype",
											"pptype_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from payrollprocesstype where code = '"+xcode+"'";
									...
									if (txtcode.getText().length() != 0) {
										...
										if(setDialog == JOptionPane.YES_OPTION){
											try {
												...
												//tablePayrollProcessType.setInsert(del_sql);

												tablePayrollProcessType.setQuery(DEFAULT_QUERY);
												int iColumn = 0;

												//if (xcode != "Code - PayrollProcessType"){
												ListSelectionModel selectionModel = tbl.getSelectionModel();
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("payrollprocesstype",
										"pptype_desc='"+DBConnect.clean(udesc)+"'",
										"pptype_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("PayrollProcessTypeMaint", "PayrollProcessTypeMaint", traceWriter.toString());      
			dispose();
		}
