 ...
 * Program: Employment Type Maintenance ==-> Payroll System
 */
...
public class EmploymentTypeMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("emptype",
														 "etype_code, etype_desc",
														 null,
														 "etype_code ASC");
	...
	private ResultSetTableModel tableEmpType;
	Container container;
	JTable tbl;
	...
	public EmploymentTypeMaint(){
		super ("Employment Type Maintenance", false, true, false, false);
		this.setName("emptype");
		...
		try{
			...
			tableEmpType  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tableEmpType.resultSet = tableEmpType.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("emptype",
								"etype_code,etype_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into emptype (emptype.code,emptype.desc,emptype.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tableEmpType.setInsert(save_sql);

								tableEmpType.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("emptype","*","etype_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("emptype",
											"etype_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from emptype where code = '"+xcode+"'";
									...
									if (txtcode.getText().length() != 0) {
										...
										if(setDialog == JOptionPane.YES_OPTION){
											try {
												...
												//tableEmpType.setInsert(del_sql);
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("emptype",
										"etype_desc='"+DBConnect.clean(udesc)+"'",
										"etype_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("EmploymentTypeMaint", "EmploymentTypeMaint", traceWriter.toString());      
			dispose();
		}
