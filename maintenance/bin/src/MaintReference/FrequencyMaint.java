 ...
 * Program: Frequency Maintenance ==-> Payroll System
 */
...
public class FrequencyMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("frequency",
														 "freq_code, freq_desc",
														 null,
														 "freq_code ASC");

	private ResultSetTableModel tablefreq;
	Container container;
	JTable tbl;
	...
	public FrequencyMaint(){
		super ("Frequency Maintenance", false, true, false, false);
		...
		try{
			...
			tablefreq  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tablefreq.resultSet = tablefreq.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("frequency",
								"freq_code,freq_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into frequency (frequency.code,frequency.desc,frequency.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tablefreq.setInsert(save_sql);

								tablefreq.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("frequency","*","freq_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("frequency",
											"freq_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from frequency where code = '"+xcode+"'";
									...
									if (txtcode.getText().length() != 0) {
										...
										if(setDialog == JOptionPane.YES_OPTION){
											try {
												...
												//tablefreq.setInsert(del_sql);

												tablefreq.setQuery(DEFAULT_QUERY);
												int iColumn = 0;

												//if (xcode != "Code - Frequency"){
												ListSelectionModel selectionModel = tbl.getSelectionModel();
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("frequency",
										"freq_desc='"+DBConnect.clean(udesc)+"'",
										"freq_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("FrequencyMaint", "FrequencyMaint", traceWriter.toString());      
			dispose();
		}
