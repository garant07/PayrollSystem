 ...
 * Program: Salary Rate Type Maintenance ==-> Payroll System
 */
...
public class SalaryRateTypeMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("salaryratetype",
														 "srtype_code, srtype_desc",
														 null,
														 "srtype_code ASC");

	private ResultSetTableModel tableSalaryRateType;
	Container container;
	JTable tbl;
	...
	public SalaryRateTypeMaint(){
		super ("Salary Rate Type Maintenance", false, true, false, false);
		this.setName("salaryratetype");
		...
		try{
			...
			tableSalaryRateType  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tbl = new JTable(tableSalaryRateType);
			...
			tableSalaryRateType.resultSet = tableSalaryRateType.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("salaryratetype",
								"srtype_code,srtype_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into salaryratetype (salaryratetype.code,salaryratetype.desc,salaryratetype.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tableSalaryRateType.setInsert(save_sql);

								tableSalaryRateType.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("salaryratetype","*","srtype_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("salaryratetype",
											"srtype_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from salaryratetype where code = '"+xcode+"'";
									...
									if (txtcode.getText().length() != 0) {
										...
										if(setDialog == JOptionPane.YES_OPTION){
											try {
												...
												//tableSalaryRateType.setInsert(del_sql);

												tableSalaryRateType.setQuery(DEFAULT_QUERY);
												int iColumn = 0;

												//if (xcode != "Code - SalaryRateType"){
												ListSelectionModel selectionModel = tbl.getSelectionModel();
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("salaryratetype",
										"srtype_desc='"+DBConnect.clean(udesc)+"'",
										"srtype_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("SalaryRateTypeMaint", "SalaryRateTypeMaint", traceWriter.toString());      
			dispose();
		}
