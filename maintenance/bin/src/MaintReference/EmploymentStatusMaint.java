 ...
 * Program: Employment Status Maintenance ==-> Payroll System
 */
...
public class EmploymentStatusMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("empstatus",
														 "estat_code, estat_desc",
														 null,
														 "estat_code ASC");
	...
	private ResultSetTableModel tableEmpStatus;
	Container container;
	JTable tbl;
	...
	public EmploymentStatusMaint(){
		super ("Employment Status Maintenance", false, true, false, false);
		this.setName("empstatus");
		...
		try{
			...
			tableEmpStatus  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tableEmpStatus.resultSet = tableEmpStatus.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("empstatus",
								"estat_code,estat_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into empstatus (empstatus.code,empstatus.desc,empstatus.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tableEmpStatus.setInsert(save_sql);

								tableEmpStatus.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("empstatus","*","estat_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("empstatus",
											"estat_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from empstatus where code = '"+xcode+"'";
									...
									if (txtcode.getText().length() != 0) {
										...
										if(setDialog == JOptionPane.YES_OPTION){
											try {
												...
												//tableEmpStatus.setInsert(del_sql);
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("empstatus",
										"estat_desc='"+DBConnect.clean(udesc)+"'",
										"estat_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("EmploymentStatusMaint", "EmploymentStatusMaint", traceWriter.toString());      
			dispose();
		}
