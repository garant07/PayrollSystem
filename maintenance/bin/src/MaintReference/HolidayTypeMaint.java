 ...
 * Program: Holiday Type Maintenance ==-> Payroll System
 */
...
public class HolidayTypeMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("holidaytype",
														 "holtype_code, holtype_desc",
														 null,
														 "holtype_code ASC");

	private ResultSetTableModel tableHolidayType;
	Container container;
	JTable tbl;
	...
	private MyField txtcode = ((MyField)new MyField(true,3));
	private MyField txtdescription = new MyField(true,15);

	public HolidayTypeMaint(){
		super ("Holiday Type Maintenance", false, true, false, false);
		this.setName("holidaytype");
		...
		try{
			...
			tableHolidayType  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tbl = new JTable(tableHolidayType);
			...
			tableHolidayType.resultSet = tableHolidayType.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("holidaytype",
								"holtype_code,holtype_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into holidaytype (holidaytype.code,holidaytype.desc,holidaytype.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tableHolidayType.setInsert(save_sql);

								tableHolidayType.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("holidaytype","*","holtype_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("holidaytype",
											"holtype_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from holidaytype where code = '"+xcode+"'";
									...
									if (txtcode.getText().length() != 0) {
										...
										if(setDialog == JOptionPane.YES_OPTION){
											try {
												...
												//tableHolidayType.setInsert(del_sql);

												tableHolidayType.setQuery(DEFAULT_QUERY);
												int iColumn = 0;

												//if (xcode != "Code - HolidayType"){
												ListSelectionModel selectionModel = tbl.getSelectionModel();
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("holidaytype",
										"holtype_desc='"+DBConnect.clean(udesc)+"'",
										"holtype_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("HolidayTypeMaint", "HolidayTypeMaint", traceWriter.toString());      
			dispose();
		}
