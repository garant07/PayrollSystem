 ...
 * Program: Cost Center Maintenance ==-> Payroll System
 */
...
public class CostCenterMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("costcenter",
														 "cost_code, cost_desc",
														 null,
														 "cost_code ASC");
	...
	private ResultSetTableModel tableCostCenter;
	Container container;
	JTable tbl;
	...
	public CostCenterMaint(){
		super ("Cost Center Maintenance", false, true, false, false);
		this.setName("costcenter");
		...
		try{
			...
			tableCostCenter  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			tbl = new JTable(tableCostCenter);
			...
			tableCostCenter.resultSet = tableCostCenter.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("costcenter",
								"cost_code,cost_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into costcenter (costcenter.code,costcenter.desc,costcenter.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tableCostCenter.setInsert(save_sql);

								tableCostCenter.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("costcenter","*","cost_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("costcenter",
											"cost_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from costcenter where code = '"+xcode+"'";
									...
									if (txtcode.getText().length() != 0) {
										...
										if(setDialog == JOptionPane.YES_OPTION){
											try {
												...
												//tableCostCenter.setInsert(del_sql);
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("costcenter",
										"cost_desc='"+DBConnect.clean(udesc)+"'",
										"cost_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("CostCenterMaint", "CostCenterMaint", traceWriter.toString());      
			dispose();
		}
