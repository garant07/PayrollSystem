 ...
 * Program: Basis of Computation Type Maintenance ==-> Payroll System
 */
...
import maintenance.bin.src.MaintEmployee.EmployeeGovtContMaint;
...
public class BasisComputationTypeMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("basiscomputationtype",
														 "bcomtype_code, bcomtype_desc",
														 null,
														 "bcomtype_code ASC");
	public static int subMain = 0;

	public static JInternalFrame iFrame = null;
	private ResultSetTableModel tableBasisComputationType;
	Container container;
	JTable tbl;
	...
	public BasisComputationTypeMaint(){
		super ("Basis of Computation Type Maintenance", false, true, false, false);
		this.setName("basiscomputationtype");
		...
		this.addInternalFrameListener(new InternalFrameListener(){
			public void internalFrameActivated(InternalFrameEvent arg0) {
				if (subMain == 1) {
					try {
						BasisComputationMaint.iFrame.setSelected(true);
					}
			...
			public void internalFrameClosing(InternalFrameEvent arg0) {
				if (subMain == 1) {
					BasisComputationMaint.iFrame.dispose();
				}
			...
			public void internalFrameDeiconified(InternalFrameEvent arg0) {
				if (subMain == 1) {
					try {
						BasisComputationMaint.iFrame.setIcon(false);
					} catch (PropertyVetoException e) {
						e.printStackTrace();
					}
				}
			...
			public void internalFrameIconified(InternalFrameEvent arg0) {
				if (subMain == 1) {
					try {
						BasisComputationMaint.iFrame.setIcon(true);
					} catch (PropertyVetoException e) {
						e.printStackTrace();
					}
				}
		...
		try{
			...
			tableBasisComputationType  = new ResultSetTableModel(  DEFAULT_QUERY );
			...
			jbtncomptn = new JButton("Comptn");
			jbtncomptn.setMnemonic('C');
			jbtncomptn.setToolTipText("Compensation");
			
			JLabel lblcode = new JLabel("Code");
			final JLabel lbldescription = new JLabel("Description");
			tbl = new JTable(tableBasisComputationType);
			...
			tableBasisComputationType.resultSet = tableBasisComputationType.statement.executeQuery(DEFAULT_QUERY);
			int colIndex0 = 0;
			int colIndex1 = 1;
			int width0 = 100;
			int width1 = 100;
			...
			jbtncomptn.setBounds(330,170,80,22);
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						jbtncomptn.setEnabled(false);
						txtcode.grabFocus();
					}
					else if (caption == "Save"){
						String save_sql = DBConnect.Insert("basiscomputationtype",
								"bcomtype_code,bcomtype_desc",
								"'"+DBConnect.clean(xcode)+"','"+DBConnect.clean(xdesc)+"'");
						//String save_sql = "Insert into basiscomputationtype (basiscomputationtype.code,basiscomputationtype.desc,basiscomputationtype.authority) values ('"+xcode+"','"+xdesc+"','"+xauth+"')";
						tbl.setEnabled(false);
						...
						try{
							if (txtcode.getText().length() == 0){   
								...
								tableBasisComputationType.setInsert(save_sql);

								tableBasisComputationType.setQuery(DEFAULT_QUERY);
								tbl.getColumnModel().getColumn(0).setMaxWidth(0);
								tbl.getColumnModel().getColumn(0).setMinWidth(0);
								tbl.getColumnModel().getColumn(1).setMaxWidth(0);
								tbl.getColumnModel().getColumn(1).setMinWidth(0);
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("basiscomputationtype","*","bcomtype_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("basiscomputationtype",
											"bcomtype_code = '"+DBConnect.clean(xcode)+"'");
									String del_sql1 = "DELETE FROM basiscomputation WHERE bcomp_code = '"+xcode+"'";
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("basiscomputationtype",
										"bcomtype_desc='"+DBConnect.clean(udesc)+"'",
										"bcomtype_code= '"+DBConnect.clean(ucode)+"'");
			...
			//Compensation button listener
			jbtncomptn.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (tbl.getRowCount() >= 1) {
						if (tbl.getSelectedRow() >= 0) {
							int getrow = tbl.getSelectedRow();
							new BasisComputationMaint(tbl.getValueAt(getrow,0).toString(),tbl.getValueAt(getrow,1).toString(),Width(),Height());
						}
					}
				}
			});
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("BasisComputationTypeMaint", "BasisComputationTypeMaint", traceWriter.toString());      
			dispose();
		}
