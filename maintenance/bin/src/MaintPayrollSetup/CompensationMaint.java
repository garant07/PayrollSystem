 ...
 * Program: Compensation Maintenance ==-> Payroll System
 */
...
public class CompensationMaint extends JInternalFrame {

	private static final long serialVersionUID = 1L;
	static final String DEFAULT_QUERY = DBConnect.Select("compensation",
														 "comps_code,comps_desc,ptype_code," +
														 "comps_taxable,comps_proctype,freq_code",
														 null,
														 "comps_code ASC");
	...
	public CompensationMaint(){
		...
		try {
			...
			while (tableCompensation.resultSet.next()){
				jcbpayrolltype.addItem(Formatter.formatString(50,' ','-',tableCompensation.resultSet.getString("ptype_desc"))+"-"+tableCompensation.resultSet.getString("ptype_code"));
			}//while (tableCompensation.resultSet.next())

			jcbtaxable.setFont(new Font("",Font.PLAIN, 12));
			jcbtaxable.setBackground(Color.white);
			jcbtaxable.addItem("Choose");
			jcbtaxable.addItem("Yes");
			jcbtaxable.addItem("No");
			...
			tableCompensation.resultSet = tableCompensation.statement.executeQuery(sql1);

			jcbfrequency.setFont(new Font("",Font.PLAIN, 12));
			jcbfrequency.setBackground(Color.white);
			jcbfrequency.addItem("Choose");
			while (tableCompensation.resultSet.next()){
				jcbfrequency.addItem(Formatter.formatString(50,' ','-',tableCompensation.resultSet.getString("freq_desc"))+"-"+tableCompensation.resultSet.getString("freq_code"));
			}//while (tableCompensation.resultSet.next())
			...
			tbl.getTableHeader().getColumnModel().getColumn(0).setMaxWidth(90);
			tbl.getTableHeader().getColumnModel().getColumn(0).setMinWidth(90);
			tbl.getTableHeader().getColumnModel().getColumn(1).setMaxWidth(335);
			tbl.getTableHeader().getColumnModel().getColumn(1).setMinWidth(335);

			tbl.getTableHeader().setResizingAllowed(false);
			tbl.getTableHeader().setMaximumSize(new Dimension(23,56));
			tbl.setSelectionMode(0);

			tbl.setPreferredScrollableViewportSize(new Dimension(425,118));

			jpan.setBounds(-39,20,560,150);

			lblcode.setBounds(17,73,100,280);
			lbldescription.setBounds(17,103,100,270);
			lblpayrolltype.setBounds(17,133,100,270);
			lbltaxable.setBounds(17,163,100,270);
			lblprocesstype.setBounds(17,193,100,270);
			lblfrequency.setBounds(17,223,100,270);

			txtcode.setBounds(119,200,80,20);
			txtcode.setDisabledTextColor(new Color(139,113,113));
			txtdescription.setBounds(119,230,350,20);
			txtdescription.setDisabledTextColor(new Color(139,113,113));
			jcbpayrolltype.setBounds(119,260,90,20);
			jcbtaxable.setBounds(119,290,80,20);
			jcbprocesstype.setBounds(119,320,100,20);
			...
			container.add(lbltaxable);
			container.add(jcbtaxable);
			...
			jcbtaxable.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					if (jcbtaxable.getSelectedItem().toString().equalsIgnoreCase("Yes") == true) {
						taxable = "Y";
					}
					else if (jcbtaxable.getSelectedItem().toString().equalsIgnoreCase("No") == true) {
						taxable = "N";
					}
					else {
						taxable = "";
			...
			jbtnadd.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					...
					if (caption == "Add"){
						...
						String save_sql = DBConnect.Insert("compensation",
								"comps_code,comps_desc,ptype_code,comps_taxable,comps_proctype,freq_code",
						...
						try {
							...
							else if (taxable.trim().length() == 0){   
								Message.messageError("Invalid input for taxable");
								jcbtaxable.grabFocus();
							}
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								try {
									...
									("compensation","*","comps_code = '" + DBConnect.clean(code) +"'");
									LockHandler.startLock(lockCmd);

									String del_sql = DBConnect.delete("compensation",
											"comps_code = '"+DBConnect.clean(xcode)+"'");
									//String del_sql = "Delete from compensation where code = '"+xcode+"'";
				...
				public void actionPerformed(ActionEvent e){
					...
					if (ucode.length() != 0){
						if (ecaption == "Edit"){
							...
							try{
								String e_sql = DBConnect.Update("compensation",
										"comps_desc='"+DBConnect.clean(udesc)+"', "+
										"ptype_code='"+ptype_code+"', "+
										"comps_taxable='"+taxable+"', "+
										"comps_proctype='"+proc_type+"', "+
										"freq_code='"+frequency+"'",
										"comps_code= '"+DBConnect.clean(ucode)+"'");
		...
		catch ( ClassNotFoundException classNotFound ) 
		{
			...
			createINI.create("CompensationMaint", "CompensationMaint", traceWriter.toString());      
			dispose();
		}
	...
	private void clearFields() {
		...
		jcbtaxable.setSelectedItem("Choose");
	...
	private void setValue() {
		...
		for (int i=0; i<=jcbpayrolltype.getItemCount()-1; i++) {
			if (jcbpayrolltype.getItemAt(i).toString().length() > 50) {
				if (tbl.getValueAt(getrow,2).toString().equalsIgnoreCase(jcbpayrolltype.getItemAt(i).toString().substring(51)) == true) {
					jcbpayrolltype.setSelectedIndex(i);
				}
			}
		}//ENDof for (int i=0; i<=jcbpayrolltype.getItemCount()-1; i++) {
		if (tbl.getValueAt(getrow,3).toString().equalsIgnoreCase("Y") == true) {
			jcbtaxable.setSelectedItem("Yes");
		}
		else {
			jcbtaxable.setSelectedItem("No");
		}
		if (tbl.getValueAt(getrow,4).toString().equalsIgnoreCase("C") == true) {
